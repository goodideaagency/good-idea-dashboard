...
export default async function ViewAsAgencyPage({
  ...
  return (
    <div>
      <div className="bg-[#f7cf4a] px-8 py-2 text-center text-xs font-semibold uppercase tracking-wide text-black font-mono">
      ...
      <div className="p-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">Viewed by admin</p>
          </div>
          <Link
            href="/admin"
        ...
        <div className="mx-auto mt-8 max-w-3xl">
          <p className="text-xs font-mono uppercase tracking-wide text-gray-400">
            Plans available to this agency
          </p>
          <div className="mt-4 bg-white p-5 ring-1 ring-[#ece7d8]">
            {plans.length === 0 ? (
              <p className="text-sm text-gray-500">No plans available for this agency yet.</p>
            ) : (
              <ul className="space-y-2">
                {plans.map((plan) => (
                  <li
                    key={plan.id}
                    className="border border-[#e7e2d3] px-3 py-2 text-sm text-gray-900"
                  >
                    {plan.label}
                  </li>
                ))}
          ...
          <p className="mt-8 text-xs font-mono uppercase tracking-wide text-gray-400">Accounts</p>
          {accounts && accounts.length > 0 ? (
            <ul className="mt-4 divide-y divide-[#f0ecdf] bg-white ring-1 ring-[#ece7d8]">
              {accounts.map((a) => {
